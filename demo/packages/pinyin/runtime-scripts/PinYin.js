...
//自定义词映射表
let PHRASES_DICT = require("dict-ci");
  ...
  convert (hans, options) {
    ...
    for (let i = 0, firstCharCode, words, l = phrases.length; i < l; i++) {
      ...
      if(PINYIN_DICT[firstCharCode]){
        ...
        if (words.length === 1) {
          pys = pys.concat(this.convert_to(words, options));
        } else {
  ...
  phrases_pinyin(phrases, options) {
    ...
    if (PHRASES_DICT.hasOwnProperty(phrases)){
      ...
      for(let i = 0, l = phrases.length; i < l; i++){
        py = py.concat(this.convert_to(phrases[i], options));
      }
    }
    return py;
  }
...
//   }
  // 词语拼音库。
  PHRASES_DICT = PHRASES_DICT || require("dict-ci");
  return [hans];//jieba.cutSmall(hans, 4);
}
